...
class PAR_New(object):
    ...
    def PAR(self, transaction):
        ...
        fp_growth = FPGrowth(itemsCol='items', minSupport=(0.0001), minConfidence=(0.0001), numPartitions=100)
        freq = fp_growth.fit(df).freqItemsets.collect()
        rules = fp_growth.fit(df).associationRules.collect()
        rule_pair = sorted(list(filter(lambda x: len(x[0]) == 1 and x[3] >= 1, rules)))

        rule_pair = {k[0]: (i[0], j) for k, i, j, z in rule_pair if k[0] != '$MISS' and i[0] != '$MISS'}
